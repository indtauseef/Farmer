...
# --- Input Fields ---
region = st.selectbox("Select Region (Sub-region of UP)", [
    "North UP", "South UP", "East UP", "West UP", "Central UP"
])
...
def recommend_crop(region, season, water, land_size, budget, previous_crop):
    if region == "West UP" and season == "Kharif":
        ...
        return random.choice(["Rice", "Wheat", "Maize"])  # Default fallback

# --- Prediction Button ---
if st.button("🔍 Get Recommendation"):
    crop = recommend_crop(region, season, water, land_size, budget, previous_crop)

    st.success(f"✅ Recommended Crop for **{region} ({season} season)**: **{crop}**")
    
    if crop in crop_images:
        st.image(crop_images[crop], caption=f"{crop} grown in {region}", use_column_width=True)
